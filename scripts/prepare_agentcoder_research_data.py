...
def main() -> None:
    ...
    if args.max_open_swe > 0:
        ...
        manifest["sources"]["nvidia/Open-SWE-Traces"] = {
            "config": args.open_swe_config,
            "records": count,
            "split": args.open_swe_split,
        }
    ...
    if args.max_swe_zero > 0:
        ...
        records = (
            openhands_row_to_record(
                row,
                source="nvidia/SWE-Zero-openhands-trajectories",
                resolved_only=False,
                languages=languages,
            )
            for row in ds
    ...
    if args.max_wildchat > 0:
        ...
        manifest["sources"]["allenai/WildChat"] = {
            "config": args.wildchat_config,
            "records": count,
            "split": args.wildchat_split,
        }
    ...
    if args.max_oasst > 0:
        ...
        manifest["sources"]["OpenAssistant/oasst1"] = {
            "config": args.oasst_config,
            "records": count,
            "split": args.oasst_split,
        }
    ...
    for subset, limit in parse_subset_limits(args.starcoder2_extras):
        ...
        records = (text_row_to_record(row, source=f"bigcode/starcoder2data-extras/{subset}") for row in ds)
        count = write_jsonl(out / "base_code_docs" / f"starcoder2_{subset}.jsonl", iter_limited(records, limit))
        manifest["sources"][f"bigcode/starcoder2data-extras/{subset}"] = {"records": count}
