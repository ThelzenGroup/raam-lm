...
def summarize_slot_families(eval_payload: dict[str, Any]) -> dict[str, dict[str, Any]]:
    buckets: dict[str, list[dict[str, Any]]] = defaultdict(list)
    for row in eval_payload.get("results", []):
        buckets[str(row.get("slot_family", "unknown"))].append(row)
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default="runs/agentcoder_slotcopy_gate")
    ...
    train_jsonl = generated_dir / "slotcopy_train.jsonl"
    cases_json = generated_dir / "slotcopy_eval_cases.json"
    data_manifest = generated_dir / "slotcopy_manifest.json"
    tokenizer = output_dir / "tokenizer.json"
    packed = output_dir / "packed"
    train_dir = output_dir / "train"
    eval_json = output_dir / "slotcopy_eval.json"
