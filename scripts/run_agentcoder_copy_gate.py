...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the copy-only AgentCoder slot binding gate.")
    parser.add_argument("--config", default="configs/scratch/raam_agentcoder_copy_gate.yaml")
    parser.add_argument("--output-dir", default="runs/agentcoder_copy_gate")
    ...
    train_jsonl = generated_dir / "copyonly_train.jsonl"
    cases_json = generated_dir / "copyonly_eval_cases.json"
    data_manifest = generated_dir / "copyonly_manifest.json"
    tokenizer = output_dir / "tokenizer.json"
    packed = output_dir / "packed"
    train_dir = output_dir / "train"
    eval_json = output_dir / "copyonly_eval.json"
