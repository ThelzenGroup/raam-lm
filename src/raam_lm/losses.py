...
def shifted_cross_entropy(logits: torch.Tensor, labels: torch.Tensor, horizon: int = 1) -> torch.Tensor:
    if logits.shape[1] <= horizon:
        return logits.sum() * 0.0
    pred = logits[:, :-horizon, :].contiguous()
    target = labels[:, horizon:].contiguous()
    return F.cross_entropy(pred.view(-1, pred.shape[-1]), target.view(-1))
...
def compute_lm_losses(
    ...
    if labels is None:
        ...
        next_loss = shifted_cross_entropy(logits, labels, horizon=1)
        total = next_loss
    ...
    if labels is not None and mtp_heads is not None and mtp_weights:
        for horizon, weight in mtp_weights.items():
            ...
            mtp_loss = shifted_cross_entropy(aux_logits, labels, horizon=horizon)
